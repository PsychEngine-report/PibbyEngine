...
function onUpdatePost()
   if followchars and startCam then
      ...
      anima = getProperty('Jake.animation.curAnim.name');
   ...
   if followchars and startCam --[[and (camOffX ~= oldCam[1] or camOffY ~= oldCam[2])]] then
      ...
      jakeX = getProperty('Jake.x');
      jakeY = getProperty('Jake.y');
      dadX = getProperty('Jake.x');
      dadY = getProperty('Jake.y');
      gfX = getProperty('gf.x');
      gfY = getProperty('gf.y');
      --debugPrint('halo');

      if mustHitSection and not gfSection and (bfPos[1] ~= nil and bfPos[2] ~= nil) then -- bf
         cameraX = getMid(bfX , getProperty('boyfriend.width')) - 150;
      ...
      if not mustHitSection and not gfSection and (jakePos[1] ~= nil and jakePos[2] ~= nil) then
         cameraX = getMid(jakeX , getProperty('Jake.width')) + 150;
         cameraY = getMid(jakeY , getProperty('Jake.height')) - 100;
         cameraX = cameraX + jakePos[1];
         cameraY = cameraY + jakePos[2];

      end

      if gfSection and (gfPos[1] ~= nil and gfPos[2] ~= nil) then -- bf
         cameraX = getMid(gfX , getProperty('gf.width')) - 800;
         cameraY = getMid(gfY , getProperty('gf.height')) - 150;
...
function findCharCam()
   ...
   cameraDad = getProperty('Jake.cameraPosition');
   dadPos[1] = cameraDad[1];
   dadPos[2] = cameraDad[2];
