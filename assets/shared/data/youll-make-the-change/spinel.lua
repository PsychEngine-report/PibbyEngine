...
downoffsets = {'-200', '-46'}
upoffsets = {'-202', '201'}
rightoffsets = {'-218', '189'}

idle_xml_name = 'Spinel idle'
left_xml_name = 'Spinel left'
down_xml_name = 'spinel down'
up_xml_name = 'spinel up'
right_xml_name = 'spinel right'
...
--setProperty('Spinel.x', (getProperty('dad.x')+100))
--setProperty('Spinel.y', (getProperty('dad.y')+0))
setObjectOrder('Spinel',getObjectOrder('dadGroup')-1)
...
function onCountdownTick(counter)
	...
	if counter % 1 == 0 then
		if holdTimers < 0 then
		--setProperty('Spinel.x', (getProperty('dad.x')+125))
--setProperty('Spinel.y', (getProperty('dad.y')+0))
			objectPlayAnimation(name_of_character, 'idle');
